...
3. Delegate independent tasks to separate Codex threads by default within this repository. Start enough workers to keep autonomous lanes moving, including separate implementation, review, proof, CI, requirements, benchmark, estimator-history, and dependency-audit lanes when those can run in parallel. Whenever assigning or materially changing work, rename the worker thread to `ARC: <short current task>`. Route coding and implementation work to `5.6 Luna` at `xhigh` reasoning. Route mathematical work to forks of the coordinator using `5.6 Sol` at `xhigh` reasoning. Do not override these assignments for review, proof, CI, requirements, benchmark, estimator-history, or dependency-audit work unless the task is substantively coding or mathematical.
4. Keep the coordinator thread lightweight. Delegate substantial implementation and independent verification to repository workers or subworkers, then monitor by reading current state.
...
- Do not implement code directly in the coordinator thread unless the change is minimal and parallel work touches only a small number of files. Use repository worker threads for all other coding, implementation, and code-modification work; keep the coordinator focused on control-plane decisions, integration, monitoring, and reporting.
- Route substantive coding and implementation tasks to `5.6 Luna` at `xhigh` reasoning. Route substantive mathematical analysis, derivations, or estimator-theory work to forks of the coordinator using `5.6 Sol` at `xhigh` reasoning. Leave other specialist lanes on the platform default unless their work is clearly coding or mathematical.
