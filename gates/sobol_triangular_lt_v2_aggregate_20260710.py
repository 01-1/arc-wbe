#!/usr/bin/env python3
"""Aggregate the preregistered Sobol triangular Stage-A v2 payload."""
...
def main() -> None:
    ...
    ratios = {
        "current_over_triangular": [a / max(b, 1e-300) for a, b in zip(mse_by_mlp["current"], mse_by_mlp["sobol_triangular"])],
        "sobol_over_triangular": [a / max(b, 1e-300) for a, b in zip(mse_by_mlp["sobol_sphere"], mse_by_mlp["sobol_triangular"])],
    }
    ratio_stats = {name: _stats(values) for name, values in ratios.items()}
    importance_shares = {str(k): _stats([float(row["importance_shares"][str(k)]) for row in rows]) for k in (1, 2, 4, 8, 16, 32)}
    ...
    gates = {
        ...
        "current_over_triangular_global": mean_mse.get("current", 0.0) / max(mean_mse.get("sobol_triangular", float("inf")), 1e-300) >= 1.25,
        "sobol_over_triangular_global": mean_mse.get("sobol_sphere", 0.0) / max(mean_mse.get("sobol_triangular", float("inf")), 1e-300) >= 1.15,
        ...
        "all_ratios_min": min(ratio_stats["current_over_triangular"].get("min", 0.0), ratio_stats["sobol_over_triangular"].get("min", 0.0)) >= 0.65,
    }
    result = {
        "script_version": "sobol-triangular-lt-v2-aggregate",
        "n_mlps": len(rows), "complete": complete, "checksums": checksums,
    ...
    lines = ["# Sobol triangular linear-transform Stage-A v2 gate", "", f"Successful shards: `{len(rows)}/100`; checksums: `{'PASS' if checksums else 'FAIL'}`.", "", f"**{result['verdict']}**", "", "| method | mean MSE | median | q10 | q90 | worst |", "|---|---:|---:|---:|---:|---:|"]
    for method in METHODS:
        stats = result["mse_stats_per_mlp"][method]
        lines.append(f"| {method} | {mean_mse[method]:.6e} | {stats['median']:.6e} | {stats['q10']:.6e} | {stats['q90']:.6e} | {stats['max']:.6e} |")
    ...
    lines += ["", f"QR max-absolute error: mean `{(float(np.mean(qr_abs)) if qr_abs else float('nan')):.6e}`, worst `{(max(qr_abs) if qr_abs else float('nan')):.6e}`.", f"QR max-relative error: mean `{(float(np.mean(qr_rel)) if qr_rel else float('nan')):.6e}`, worst `{(max(qr_rel) if qr_rel else float('nan')):.6e}`.", "", "## Stage-A gate decisions", ""]
