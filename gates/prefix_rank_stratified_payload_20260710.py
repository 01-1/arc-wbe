#!/usr/bin/env python3
"""Prefix-ranked stratified suffix research payload (truth delayed)."""
...
KS = (2, 4, 8, 12)
TERMINAL_BLOCKS = {2: 13, 4: 11, 8: 8, 12: 3}
...
SCRIPT_VERSION = "prefix-rank-stratified-v2-corrected"
...
def _suffix_final(raw: np.ndarray, weights: list[fnp.ndarray], k: int) -> np.ndarray:
    ...
    return _pair_fold(np.asarray(x, dtype=np.float64))
...
def _fit_predict(pilot_x: np.ndarray, pilot_t: np.ndarray, main_x: np.ndarray) -> tuple[np.ndarray, float, float]:
    ...
    centered_t = pilot_t - np.mean(pilot_t)
    gram = centered_x.T @ centered_x
    ridge = 0.1 * float(np.trace(gram)) / WIDTH
    beta = np.linalg.solve(gram + ridge * np.eye(WIDTH), centered_x.T @ centered_t)
    pred = np.mean(pilot_t) + (main_z - np.mean(pilot_z, axis=0)[None, :]) @ beta
...
def _candidate_for_k(
    ...
    raw: np.ndarray,
    folded: np.ndarray,
    pilot_final: np.ndarray,
) -> dict[str, object]:
    pilot_x = folded[:N_PILOT]
    main_x = folded[N_PILOT:]
    u = np.mean(pilot_final, axis=0)
    u /= max(float(np.linalg.norm(u)), _MIN_VARIANCE)
    pilot_mean = np.mean(pilot_final, axis=0)
    pilot_t = pilot_final @ u
    pilot_mean_s = float(np.mean(pilot_t))
    pred, ridge, condition = _fit_predict(pilot_x, pilot_t, main_x)
    selected_local, inclusion_weights, stratum_sizes = _select_stratified(pred, seed, k)
    ...
    pbar_main = pilot_mean + (float(np.mean(pred)) - pilot_mean_s) * u
    selected_predictor = pilot_mean[None, :] + (pred[selected_local] - pilot_mean_s)[:, None] * u[None, :]
    ...
    control_predictor = pilot_mean[None, :] + (pred[control_local] - pilot_mean_s)[:, None] * u[None, :]
    ...
    actual_selected_t = selected_final @ u
    selected_corr = float(np.corrcoef(pred[selected_local], actual_selected_t)[0, 1]) if len(selected_local) > 1 else 0.0
    return {
        "k": k,
        "candidate": candidate,
        "raw_rank": raw_rank,
        ...
        "compute_dense_ratio": float((POOL_BLOCKS * k + TERMINAL_BLOCKS[k] * (32 - k)) / (CURRENT_BLOCKS * 32)),
        "projected_raw_flops": float(2.535e10 * (POOL_BLOCKS * k + TERMINAL_BLOCKS[k] * (32 - k)) / (CURRENT_BLOCKS * 32)),
    }
...
def run_one(shard_index: int, bank_path: Path, reps: int) -> dict[str, object]:
    ...
    for rep in range(reps):
        ...
        pilot_rows = np.concatenate((raw_by_k[12][:N_PILOT], raw_by_k[12][N_PAIRS : N_PAIRS + N_PILOT]), axis=0)
        pilot_final = _suffix_final(pilot_rows, weights, 12)
        candidates = {
            k: _candidate_for_k(route_seed, k, weights, raw_by_k[k], folded_by_k[k], pilot_final)
            for k in KS
        }
        ...
        for k, result in candidates.items():
            ...
            actual_main_t = main_final @ result["_u"]
            result["main_predictor_corr"] = float(np.corrcoef(result["_pred_main"], actual_main_t)[0, 1]) if len(actual_main_t) > 1 else 0.0
        current = _full_route_mean(mlp, route_seed, CURRENT_BLOCKS)
        full_rows = np.concatenate((raw_by_k[12][:N_PAIRS], raw_by_k[12][N_PAIRS:]), axis=0)
        full_pool_mean = np.mean(_suffix_final(full_rows, weights, 12), axis=0)
    ...
    for fixed in fixed_reps:
        ...
        for k, result in fixed["candidates"].items():
            ...
            k_results[str(k)] = {
                key: value for key, value in result.items() if key not in {"candidate", "control", "raw_rank", "_pred_main", "_u"}
            }
