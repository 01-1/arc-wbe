#!/usr/bin/env python3
"""Aggregate the preregistered paired Sobol-sphere Fly gate."""
...
def main() -> None:
    ...
    result = {
        "script_version": "sobol-sphere-aggregate-v1",
        "n_mlps": len(rows),
        "complete": complete,
        "checksums": checksums,
    ...
    if not rows:
        lines.append("No 3-rep estimates were returned; the Fly run failed before payload import.")
    if rows:
        for method in METHODS:
