# the reference entrant Contraction Closeout, 2026-07-07

Scope: focused follow-up on the reference entrant's hidden-profile contraction clue and the
...
Several older the reference entrant/readout/collapse gate artifacts are gitignored in this
worktree, so the durable history entries were treated as the source of truth.

## What Remains True

The the reference entrant gate found one real physical signal: injected mean-relevant hidden
errors contract at about `0.943` per layer, matching the reference entrant's public hidden
...
The remaining anomaly is terminal, not hidden: the reference entrant's profile shows an
...
the reference entrant's hidden contraction clue remains real but non-discriminating. It
...
1. a better shape-preserving sampler constant, suggested by the shape of
   several leading entries but not recoverable from a hidden profile alone;
