...
1. Read `AGENTS.md` and `docs/how-to/estimator-history.md` before judging estimator behavior.
...
- Shape- or budget-aware routing based on `mlp.width`, `mlp.depth`, `len(mlp.weights)`, or `budget`, when the choice is a general algorithmic/resource tradeoff rather than benchmark special-casing.
