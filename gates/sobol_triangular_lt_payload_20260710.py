#!/usr/bin/env python3
"""Fly payload for the preregistered Sobol triangular Stage-A gate."""
...
SCRIPT_VERSION = "sobol-triangular-lt-v1"
...
def _sobol_sphere_rows(seed: int, width: int) -> np.ndarray:
    ...
    uniforms = np.clip(sampler.random_base2(m=12), UNIFORM_LO, UNIFORM_HI)
...
def _pilot_importance(
    ...
    """Return squared-gradient importance, pilots, and Rademacher probes."""
    ...
    return importance, order, pilots
...
def run_one(shard_index: int, bank_path: Path) -> dict[str, object]:
    ...
    sobol_rows = _sobol_sphere_rows(_derived_seed(seed, SOBOL_STREAM), WIDTH)
    importance, order, _ = _pilot_importance(weights_f32, seed)
    triangular_rows, qr_error, _ = _triangular_rows(sobol_rows, weights, order)
    estimates = {
    ...
    return {
        ...
        "config": {
            ...
            "pilots": PILOTS,
            "probes": PROBES,
            "sobol_transform": "shared qmc.Sobol.random_base2(m=12)->ndtri->row_normalize*sqrt(d)",
